...
impl LineIndex {
    pub fn build(corpus: impl AsRef<str>) -> Self {
        ...
        if corpus.is_empty() {
            ...
            let mut nl_indices = chunk_str(corpus, num_cpus::get())
                .par_bridge()
            ...
            if nl_indices.last().copied() != Some(corpus.len()) {
                nl_indices.push(corpus.len());
            }
            LineIndex {
                line_ends: nl_indices.into(),
            }
        }
    }
